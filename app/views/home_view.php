...
">
                ...
                <select placeholder="Any Specialization" name="specialization" id="Specialization" class="holder">
                    <option value=""></option>
                    <option value="0" data-area="0" selected="selected">Any Specialization</option>
    ...
    <div class="box-container">
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const yyyyMax = twoWeeksFromNow.getFullYear();
                const mmMax = String(twoWeeksFromNow.getMonth() + 1).padStart(2, '0');
                const ddMax = String(twoWeeksFromNow.getDate()).padStart(2, '0');
                const formattedMaxDate = `${yyyyMax}-${mmMax}-${ddMax}`;

                // Set max date to two weeks from today
                dateInput.setAttribute('max', formattedMaxDate);

                // Optional: Set default value to today
                dateInput.value = formattedToday;

                // Ensure user cannot input an invalid date
                dateInput.addEventListener('change', function() {
                    if (this.value < formattedToday) {
                        this.value = formattedToday;
                    } else if (this.value > formattedMaxDate) {
                        this.value = formattedMaxDate;
                    }
                });
            });
        </script>
